...
export default function HomePage() {
  ...
  const handleOrderSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitted(true);
    setOrderForm({ name: '', phone: '', address: '', volume: '' });
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="mt-6 md:mt-8 grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            <div className="text-center bg-white/10 backdrop-blur-sm p-5 md:p-6 rounded-xl">
              ...
              <h3 className="font-bold mb-1 md:mb-2 text-sm md:text-base" style={{ color: colors.text }}>შერჩევა</h3>
              <p className="text-xs md:text-sm" style={{ color: colors.light }}>საუკეთესო ხარისხის ინგრედიენტების შერჩევა</p>
            </div>
            <div className="text-center bg-white/10 backdrop-blur-sm p-5 md:p-6 rounded-xl">
              <div className="text-3xl md:text-4xl mb-2 md:mb-3">2️⃣</div>
              <h3 className="font-bold mb-1 md:mb-2 text-sm md:text-base" style={{ color: colors.text }}>დამუშავება</h3>
              <p className="text-xs md:text-sm" style={{ color: colors.light }}>ხელით დამუშავება ტრადიციული მეთოდით</p>
            </div>
            <div className="text-center bg-white/10 backdrop-blur-sm p-5 md:p-6 rounded-xl">
              <div className="text-3xl md:text-4xl mb-2 md:mb-3">3️⃣</div>
              <h3 className="font-bold mb-1 md:mb-2 text-sm md:text-base" style={{ color: colors.text }}>შეფუთვა</h3>
              <p className="text-xs md:text-sm" style={{ color: colors.light }}>სანიტარული ნორმების დაცვით შეფუთვა</p>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="bg-white rounded-xl md:rounded-2xl p-6 md:p-8 shadow-2xl">
            ...
            {submitted ? (
              <div className="text-center py-8">
                ...
                <h4 className="text-xl font-bold mb-2" style={{ color: colors.primary }}>შეკვეთა მიღებულია!</h4>
                <p className="text-sm" style={{ color: colors.textDark }}>მალე დაგიკავშირდებით</p>
              </div>
            ) : (
              <form onSubmit={handleOrderSubmit} className="space-y-5 md:space-y-6">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full py-4 rounded-lg font-bold text-lg md:text-xl hover:opacity-90 transition-opacity shadow-lg disabled:opacity-50"
                  style={{ backgroundColor: colors.primary, color: colors.text }}
                >
                  {submitting ? 'იგზავნება...' : 'შეკვეთის გაფორმება'}
                </button>
              </form>
            )}
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
            <div className="text-center bg-white/10 backdrop-blur-sm p-6 md:p-8 rounded-xl shadow-lg">
              ...
              <h3 className="text-lg md:text-xl font-bold mb-2 md:mb-3" style={{ color: colors.text }}>სწრაფი მიწოდება</h3>
              <p className="text-sm" style={{ color: colors.light }}>თბილისში იმავე დღეს (17:00-მდე შეკვეთისას)</p>
            </div>
            <div className="text-center bg-white/10 backdrop-blur-sm p-6 md:p-8 rounded-xl shadow-lg">
              <div className="text-5xl md:text-6xl mb-3 md:mb-4">🌱</div>
              <h3 className="text-lg md:text-xl font-bold mb-2 md:mb-3" style={{ color: colors.text }}>100% ნატურალური</h3>
              <p className="text-sm" style={{ color: colors.light }}>ქიმიური დანამატებისა და კონსერვანტების გარეშე</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer className="py-8 md:py-12 px-4 md:px-8" style={{ backgroundColor: colors.footer }}>
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 md:gap-8 mb-6 md:mb-8">
            ...
            <div>
              <h4 className="font-bold mb-3 md:mb-4 text-sm md:text-base" style={{ color: colors.text }}>ნავიგაცია</h4>
              <ul className="space-y-1 md:space-y-2 text-xs md:text-sm" style={{ color: colors.light }}>
                <li><a href="#intro" className="hover:opacity-80 transition-opacity">მთავარი</a></li>
                <li><a href="#video" className="hover:opacity-80 transition-opacity">ვიდეო</a></li>
                <li><a href="#order" className="hover:opacity-80 transition-opacity">შეკვეთა</a></li>
              </ul>
            </div>
            <div>
              <h4 className="font-bold mb-3 md:mb-4 text-sm md:text-base" style={{ color: colors.text }}>კონტაქტი</h4>
              <ul className="space-y-1 md:space-y-2 text-xs md:text-sm" style={{ color: colors.light }}>
                <li className="flex items-center gap-2"><Phone size={14} /> +995 555 123 456</li>
                <li>📧 info@ajika.store</li>
                <li className="flex items-center gap-2"><MapPin size={14} /> თბილისი</li>
              </ul>
            </div>
            <div>
              <h4 className="font-bold mb-3 md:mb-4 text-sm md:text-base" style={{ color: colors.text }}>სამუშაო საათები</h4>
              <ul className="space-y-1 md:space-y-2 text-xs md:text-sm" style={{ color: colors.light }}>
